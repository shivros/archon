...
type Model struct {
	...
	guidedWorkflowInteractiveStateTransitionGateway GuidedWorkflowInteractiveStateTransitionGateway
	guidedWorkflowPreviewStateTransitionGateway     GuidedWorkflowPreviewStateTransitionGateway
...
func (m *Model) dismissWorkflowRunLocally(runID string) bool {
	...
	if m.guidedWorkflow != nil && strings.TrimSpace(m.guidedWorkflow.RunID()) == runID {
		m.guidedWorkflowInteractiveStateTransitionGatewayOrDefault().ApplyRun(run)
	}
...
func (m *Model) applySidebarItemsWithReason(reason sidebarApplyReason) {
	...
	if !item.isSession() {
		...
		if item.kind == sidebarWorkflow && m.mode != uiModeGuidedWorkflow {
			m.renderWorkflowPreview(item)
			return
		}
		if m.mode != uiModeGuidedWorkflow {
...
func (m *Model) handleTick(msg tickMsg) tea.Cmd {
	...
	if cmd := m.maybeAutoRefreshWorkflowPreview(now); cmd != nil {
		cmds = append(cmds, cmd)
	}
