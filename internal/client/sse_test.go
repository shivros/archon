...
func TestEventStreamParsesEvents(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	ch, stop, err := c.EventStream(ctx, "s1")
	if err != nil {
	...
	case <-time.After(time.Second):
		t.Fatalf("timeout waiting for events stream payload")
	}
}

func TestItemsStreamParsesEvents(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "Bearer token" {
			w.WriteHeader(http.StatusUnauthorized)
			return
		...
		w.Header().Set("Content-Type", "text/event-stream")
		flusher, _ := w.(http.Flusher)
		item := map[string]any{"type": "assistant", "text": "ok"}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	ch, stop, err := c.ItemsStream(ctx, "s1")
	if err != nil {
		t.Fatalf("ItemsStream: %v", err)
	}
	defer stop()
	select {
	case item := <-ch:
		if typ, _ := item["type"].(string); typ != "assistant" {
			t.Fatalf("unexpected item: %#v", item)
		}
	case <-time.After(time.Second):
		t.Fatalf("timeout waiting for items stream payload")
	}
}

func TestEventStreamReturnsAPIErrorOnNon2xx(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"error":"unauthorized"}`))
	}))
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if _, _, err := c.EventStream(ctx, "s1"); err == nil {
		t.Fatalf("expected non-2xx error")
	}
}

func TestItemsStreamReturnsAPIErrorOnNon2xx(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"error":"unauthorized"}`))
	}))
